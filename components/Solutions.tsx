...
interface SolutionCardProps {
  ...
  features: string[];
  gradient: string;
  accentColor: string; // "cyan-400" | "emerald-400" | "indigo-400" | "purple-400" | "orange-400"
  delay?: string;
  floatDelay?: string;
}
...
const SolutionCard: React.FC<SolutionCardProps> = ({ title, description, icon, features, gradient, accentColor, delay, floatDelay = "0s" }) => {
  ...
  // Mapeamento explícito para evitar problemas com JIT do Tailwind
  const colorMap: Record<string, { dot: string; text: string; shadow: string; glow: string }> = {
    "cyan-400": { 
        dot: "bg-cyan-400", 
        text: "text-cyan-400", 
        shadow: "shadow-[0_0_20px_2px_rgba(34,211,238,0.8)]",
        glow: "bg-cyan-400"
    },
    "emerald-400": { 
        dot: "bg-emerald-400", 
        text: "text-emerald-400", 
        shadow: "shadow-[0_0_20px_2px_rgba(52,211,153,0.8)]",
        glow: "bg-emerald-400"
    },
    "indigo-400": { 
        dot: "bg-indigo-400", 
        text: "text-indigo-400", 
        shadow: "shadow-[0_0_20px_2px_rgba(129,140,248,0.8)]",
        glow: "bg-indigo-400"
    },
    "purple-400": { 
        dot: "bg-purple-400", 
        text: "text-purple-400", 
        shadow: "shadow-[0_0_20px_2px_rgba(192,132,252,0.8)]",
        glow: "bg-purple-400"
    },
    "orange-400": { 
        dot: "bg-orange-400", 
        text: "text-orange-400", 
        shadow: "shadow-[0_0_20px_2px_rgba(251,146,60,0.8)]",
        glow: "bg-orange-400"
    },
  };

  const currentColors = colorMap[accentColor] || colorMap["cyan-400"];
    ...
    >
      ...
      <div className="relative h-full p-6 lg:p-8 flex flex-col z-10">
        ...
        <div className="absolute top-0 left-0 w-full z-20">
             <div className={`w-full h-[2px] bg-gradient-to-r ${gradient} ${currentColors.shadow} opacity-90`} />
        ...
        <div className="flex justify-between items-start mb-6">
            <motion.div 
                ...
                className={`p-3 rounded-xl bg-white/5 border border-white/5 ${currentColors.text} relative group-hover:scale-105 transition-transform duration-500`}
            >
                <div className={`absolute inset-0 ${currentColors.glow} opacity-0 group-hover:opacity-20 blur-lg transition-opacity duration-500 rounded-xl`} />
                <span className="relative z-10">{icon}</span>
            </motion.div>

            <div className="flex items-center gap-2 px-2.5 py-1 rounded-full bg-black/40 border border-white/10 backdrop-blur-sm" role="status" aria-label="Agente Online">
                <span className="relative flex h-1.5 w-1.5">
                    <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${currentColors.dot} opacity-75`}></span>
                    <span className={`relative inline-flex rounded-full h-1.5 w-1.5 ${currentColors.dot}`}></span>
                </span>
