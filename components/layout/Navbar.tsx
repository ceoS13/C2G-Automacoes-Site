        ...
        <nav className="relative bg-[#0a0a0a]/80 backdrop-blur-lg border border-white/10 rounded-full pl-4 pr-2 py-2 shadow-xl shadow-black/50 flex items-center justify-between transition-all duration-300 z-50">
          ...
          <div className="shrink-0 flex items-center">
            <motion.button
              ...
              onClick={() => scroll.scrollToTop({ duration: 200, smooth: true })}
              whileHover={{
                scale: 1.1,
            ...
            <div className="flex items-center gap-1 bg-white/5 border border-white/5 rounded-full px-2 py-1">
              {NAV_LINKS.map((link) => (
                <Link
                  ...
                  spy={true}
                  smooth={true}
                  offset={link.offset} // Offset individual por seção
                  duration={600}
          ...
          <div className="shrink-0 flex items-center gap-3">
            ...
            <button
              type="button"
              onClick={toggleMenu}
              className={`md:hidden p-2.5 border rounded-full transition-all focus:outline-none active:scale-95 z-50 relative ${isOpen
                    ...
                    {NAV_LINKS.map((link) => (
                      <motion.div key={link.name} variants={itemVariants}>
                        <Link
                          ...
                          spy={true}
                          smooth={true}
                          offset={link.offset} // Offset individual
                          duration={600}
